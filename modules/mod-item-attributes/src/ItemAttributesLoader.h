...
struct ItemAttributeTemplate
{
    ...
    int256 minPercent;          // 属性值的最小百分比
    int256 maxPercent;          // 属性值的最大百分比
...
public:
    ...
    // 物品属性应用和移除
    ItemAttributeResult ApplyAttributeToItem(Item* item, uint32 attributeId, Player* player = nullptr, AttributeCategory category = AttributeCategory::ADDITIONAL, int256 minValue = 0, int256 maxValue = 0);
    ...
    bool GetItemAttributesWithValues(Item* item, std::vector<uint32>& attributes, std::vector<int256>& values, uint32* itemId = nullptr) const;
    std::vector<uint32> GetItemAttributes(Item* item) const;
    std::vector<int256> GetItemAttributeValues(Item* item) const;

    // 物品属性计算
    int256 CalculateAttributeValue(Item* item, ItemAttributeTemplate const* attributeTemplate) const;
    int256 CalculateAttributeValueWithRange(Item* item, ItemAttributeTemplate const* attributeTemplate, int256 minValue, int256 maxValue) const;
