...
namespace
{
float ToStatFloat(int256 const& value)
{
    return Acore::Number::ToFloat(value);
}

int256 ToRatingValue(int256 const& value)
{
    return value;
}

int32 ToLegacyInt32(int256 const& value)
{
    return Acore::Number::ToInt32Saturated(value);
}

uint32 ToSettingUInt32(int256 const& value)
{
    if (value <= 0)
        return 0;
...
void ItemAttributesEffects::Initialize()
{
    ...
    RegisterAttributeEffectHandler(4,
        // 应用效果
        [](Player* player, Item* item, ItemAttributeTemplate const* attributeTemplate, int256 value) {
            // 【关键修复】安全获取物品GUID用于日志
            uint64 itemGuid = 0;
            if (item) {
        ...
        // 生成描述
        [](Item* item, ItemAttributeTemplate const* attributeTemplate, int256 value) {
