...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    auto totalMs = std::chrono::duration_cast<std::chrono::milliseconds>(LogoutClock::now() - logoutStart).count();
    if (totalMs >= LOGOUT_TOTAL_WARN_MS)
        LOG_WARN("server.loading",
            "[性能监控-登出总耗时] 账号={} 总耗时={}ms (阈值{}ms)",
            GetAccountId(), totalMs, LOGOUT_TOTAL_WARN_MS);
    else
        LOG_INFO("server.loading",
            "[性能监控-登出总耗时] 账号={} 总耗时={}ms",
            GetAccountId(), totalMs);
}
