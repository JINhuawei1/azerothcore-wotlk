...
uint32 ToPetUpdateFieldValue(uint256 const& value)
{
...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current, uint32 healthPct /*= 0*/, bool fullMana /*= false*/)
{
        ...
        (SQLQueryHolderBase const& holder)
    {
        ...
        uint256 curHealth = savedhealth;
        if (healthPct)
        {
            curHealth = CountPctFromMaxHealth256(healthPct);
        }

        uint256 curMana = savedmana;
        if (fullMana)
        ...
        else
        {
            ...
            {
                SetHealthForCombat256(curHealth);
...
void Pet::SavePetToDB(PetSaveMode mode)
{
    ...
    uint256 curhealth = GetHealthForCombat256();
    uint256 curmana = GetPowerForCombat256(POWER_MANA);
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (pInfo)                                      // exist in DB
    {
        ...
        uint256 health64 = std::max<uint256>(1, Acore::Number::ToUInt256Saturated(Acore::Number::ToLongDouble(pInfo->health) * static_cast<long double>(factorHealth)));
    ...
    else                                            // not exist in DB, use some default fake data
    {
        ...
        uint256 health64 = std::max<uint256>(1, Acore::Number::ToUInt256Saturated(Acore::Number::ToLongDouble(stats->BaseHealth[cinfo->expansion]) * static_cast<long double>(factorHealth)));
        uint32 health = ToPetUpdateFieldValue(health64);
        m_extendedCreateHealth = health64;
        SetCreateHealth(health);
        SetModifierValue(UNIT_MOD_HEALTH, BASE_VALUE, Acore::Number::ToFloat(health64));
        ...
        uint256 mana64 = Acore::Number::ToUInt256Saturated(Acore::Number::ToLongDouble(stats->BaseMana) * static_cast<long double>(factorMana));
        uint32 mana = ToPetUpdateFieldValue(mana64);
        m_extendedCreateMana = mana64;
        SetCreateMana(mana);
        SetModifierValue(UNIT_MOD_MANA, BASE_VALUE, Acore::Number::ToFloat(mana64));
...
void Pet::FillPetInfo(PetStable::PetInfo* petInfo) const
{
    ...
    petInfo->Health = GetHealthForCombat256();
    petInfo->Mana = GetPowerForCombat256(POWER_MANA);
