    ...
    if (index == UNIT_FIELD_RANGED_ATTACK_POWER && GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (player)
        {
            ...
            if (result)
            {
                ...
                uint256 apLimit256 = fields[0].GetUInt256();
                int32 apLimit = Acore::Number::ToInt32Saturated(Acore::Number::ToInt256Saturated(apLimit256));
                LOG_INFO("entities.player", "远程攻击强度拦截: 职业={}, 原值={}, 上限={}", player->getClass(), value, apLimit256.convert_to<std::string>());
