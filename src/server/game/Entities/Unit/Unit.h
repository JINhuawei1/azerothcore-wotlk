...
struct CleanDamage
{
    CleanDamage(uint256 const& mitigated, uint256 const& absorbed, WeaponAttackType _attackType, MeleeHitOutcome _hitOutCome) :
        absorbed_damage(absorbed), mitigated_damage(mitigated), attackType(_attackType), hitOutCome(_hitOutCome) {}

    uint256 absorbed_damage;
    uint256 mitigated_damage;
...
private:
    ...
    uint256 m_resist;
    uint256 m_block;
    uint256 m_cleanDamage;

    // amalgamation constructor (used for proc)
    DamageInfo(DamageInfo const& dmg1, DamageInfo const& dmg2);

public:
    explicit DamageInfo(Unit* _attacker, Unit* _victim, uint256 const& _damage, SpellInfo const* _spellInfo, SpellSchoolMask _schoolMask, DamageEffectType _damageType, uint256 const& cleanDamage = 0);
    ...
    [[nodiscard]] uint256 const& GetDamage() const { return m_damage; };
    [[nodiscard]] uint256 const& GetAbsorb() const { return m_absorb; };
    [[nodiscard]] uint256 const& GetResist() const { return m_resist; };
    [[nodiscard]] uint256 const& GetBlock() const { return m_block; };

    [[nodiscard]] uint256 GetUnmitigatedDamage() const;
};
...
private:
    ...
    Unit* const m_target;
    uint256 m_heal;
    uint256 m_effectiveHeal;
    uint256 m_absorb;
    SpellInfo const* const m_spellInfo;
    SpellSchoolMask const m_schoolMask;
public:
    explicit HealInfo(Unit* _healer, Unit* _target, uint256 const& _heal, SpellInfo const* _spellInfo, SpellSchoolMask _schoolMask)
    ...
    void AbsorbHeal(uint256 const& amountIn)
    {
    ...
    void SetHeal(uint256 const& amount)
    {
        m_heal = amount;
    }

    void SetEffectiveHeal(uint256 const& amount)
    {
        m_effectiveHeal = amount;
    }
    ...
    [[nodiscard]] uint256 const& GetHeal() const { return m_heal; }
    [[nodiscard]] uint256 const& GetEffectiveHeal() const { return m_effectiveHeal; }
    [[nodiscard]] uint256 const& GetAbsorb() const { return m_absorb; }
    [[nodiscard]] SpellInfo const* GetSpellInfo() const { return m_spellInfo; };
    [[nodiscard]] SpellSchoolMask GetSchoolMask() const { return m_schoolMask; };
};
...
struct CalcDamageInfo
{
    ...
    uint32 procVictim;
    uint32 procEx;
    uint256 cleanDamage;          // Used only for rage calculation
...
{
    SpellPeriodicAuraLogInfo(AuraEffect const* _auraEff, uint256 const& _damage, uint256 const& _overDamage, uint256 const& _absorb, uint256 const& _resist, float _multiplier, bool _critical)
    ...
    uint256 damage;
    uint256 overDamage;                                      // overkill/overheal
    uint256 absorb;
    uint256 resist;
    float  multiplier;
    bool   critical;
...
public:
    ...
    [[nodiscard]] virtual uint256 GetHealthForCombat256() const { return GetHealth(); }
    [[nodiscard]] virtual uint256 GetMaxHealthForCombat256() const { return GetMaxHealth(); }
    ...
    [[nodiscard]] float GetHealthPct() const { uint256 maxHealth = GetMaxHealthForCombat256(); return maxHealth != 0 ? float(100.0L * Acore::Number::ToLongDouble(GetHealthForCombat256()) / Acore::Number::ToLongDouble(maxHealth)) : 0.0f; }
    int64 GetHealthGain(int64 dVal);
    [[nodiscard]] uint32 GetCreateHealth() const { return GetUInt32Value(UNIT_FIELD_BASE_HEALTH); }
    [[nodiscard]] virtual uint256 GetCreateHealthForCombat256() const { return GetCreateHealth(); }
    [[nodiscard]] virtual uint64 GetCreateHealthForCombat() const { return GetCreateHealth(); }

    [[nodiscard]] bool IsFullHealth() const { return GetHealthForCombat256() >= GetMaxHealthForCombat256(); }

    [[nodiscard]] bool HealthBelowPct(int32 pct) const { return GetHealthForCombat256() < CountPctFromMaxHealth256(pct); }
    [[nodiscard]] bool HealthBelowPctDamaged(int32 pct, uint64 damage) const { return HealthBelowPctDamaged(pct, static_cast<uint256>(damage)); }
    [[nodiscard]] bool HealthBelowPctDamaged(int32 pct, uint256 damage) const { uint256 health = GetHealthForCombat256(); return health <= damage || health - damage < CountPctFromMaxHealth256(pct); }
    [[nodiscard]] bool HealthAbovePct(int32 pct) const { return GetHealthForCombat256() > CountPctFromMaxHealth256(pct); }
    [[nodiscard]] bool HealthAbovePctHealed(int32 pct, uint256 const& heal) const { uint256 health = GetHealthForCombat256(); uint256 threshold = CountPctFromMaxHealth256(pct); return health > threshold || heal > threshold - health; }

    [[nodiscard]] uint256 CountPctFromMaxHealth256(int32 pct) const { return Acore::Number::CalculatePct(GetMaxHealthForCombat256(), pct); }
    [[nodiscard]] uint256 CountPctFromCurHealth256(int32 pct) const { return Acore::Number::CalculatePct(GetHealthForCombat256(), pct); }
    [[nodiscard]] uint64 CountPctFromMaxHealth(int32 pct) const { return Acore::Number::ToUInt64Saturated(CountPctFromMaxHealth256(pct)); }
    [[nodiscard]] uint64 CountPctFromCurHealth(int32 pct) const { return Acore::Number::ToUInt64Saturated(CountPctFromCurHealth256(pct)); }

    void SetHealth(uint32 val);
    void SetHealthForCombat(uint64 value);
    void SetHealthForCombat256(uint256 value);
    ...
    [[nodiscard]] virtual uint256 GetPowerForCombat256(Powers power) const { return GetPower(power); }
    [[nodiscard]] virtual uint256 GetMaxPowerForCombat256(Powers power) const { return GetMaxPower(power); }
    ...
    [[nodiscard]] float GetPowerPct(Powers power) const { uint256 maxPower = GetMaxPowerForCombat256(power); return maxPower != 0 ? float(100.0L * Acore::Number::ToLongDouble(GetPowerForCombat256(power)) / Acore::Number::ToLongDouble(maxPower)) : 0.0f; }
    [[nodiscard]] uint32 GetCreatePowers(Powers power) const;
    [[nodiscard]] virtual uint256 GetCreatePowerForCombat256(Powers power) const { return GetCreatePowers(power); }
    ...
    void SetPowerForCombat256(Powers power, uint256 value, bool withPowerUpdate = true);
    void SetMaxPower(Powers power, uint32 val);

    int32 ModifyPower(Powers power, int32 val, bool withPowerUpdate = true);
    int64 ModifyPower64(Powers power, int64 val, bool withPowerUpdate = true);
    int256 ModifyPower256(Powers power, int256 const& val, bool withPowerUpdate = true);
    int32 ModifyPowerPct(Powers power, float pct, bool apply = true);

    void RewardRage(uint32 damage, uint32 weaponSpeedHitFactor, bool attacker);

    [[nodiscard]] uint32 GetCreateMana() const { return GetUInt32Value(UNIT_FIELD_BASE_MANA); }
    [[nodiscard]] virtual uint256 GetCreateManaForCombat256() const { return GetCreateMana(); }
    ...
    [[nodiscard]] int256 GetExtendedWeaponDamageRange256(WeaponAttackType attType, WeaponDamageRange type) const;
    void SetExtendedWeaponDamageRange256(WeaponAttackType attType, WeaponDamageRange damageRange, int256 const& value) { m_extendedWeaponDamage[attType][damageRange] = value > 0 ? value : 0; }
    ...
    /*********************************************************/
    static uint256 DealDamage(Unit* attacker, Unit* victim, uint256 const& damage, CleanDamage const* cleanDamage = nullptr, DamageEffectType damagetype = DIRECT_DAMAGE, SpellSchoolMask damageSchoolMask = SPELL_SCHOOL_MASK_NORMAL, SpellInfo const* spellProto = nullptr, bool durabilityLoss = true, bool allowGM = false, Spell const* spell = nullptr);
    void DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss);
    void DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss, Spell const* spell = nullptr);
    static void DealDamageMods(Unit const* victim, uint256& damage, uint256* absorb);
    static void DealDamageMods(Unit const* victim, uint64& damage, uint64* absorb);
    static void DealDamageMods(Unit const* victim, uint32& damage, uint32* absorb);
    [[nodiscard]] uint256 GetCustomTrueDamageBonus() const;
    [[nodiscard]] uint256 GetCustomCuttingDamageBonus() const;
    [[nodiscard]] uint256 GetCustomSkillDamageBonus() const;
    ...
    // Calculate methods
    uint256 CalculateDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, uint8 itemDamagesMask = 0);
    ...
    uint256 MeleeDamageBonusTaken(Unit* attacker, uint256 const& pdamage, WeaponAttackType attType, SpellInfo const* spellProto = nullptr, SpellSchoolMask damageSchoolMask = SPELL_SCHOOL_MASK_NORMAL);

    // Spell damage bonus
    int32 SpellBaseDamageBonusDone(SpellSchoolMask schoolMask);
    int32 SpellBaseDamageBonusTaken(SpellSchoolMask schoolMask, bool isDoT = false);
    float SpellPctDamageModsDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype);
    uint256 SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint256 const& pdamage, DamageEffectType damagetype, uint8 effIndex, float TotalMod = 0.0f, uint32 stack = 1);
    uint256 SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint256 const& pdamage, DamageEffectType damagetype, uint32 stack = 1);

    // AOE damages
    uint256 CalculateAOEDamageReduction(uint256 const& damage, uint32 schoolMask, bool npcCaster) const;

    // Armor reduction
    static bool IsDamageReducedByArmor(SpellSchoolMask damageSchoolMask, SpellInfo const* spellInfo = nullptr, uint8 effIndex = MAX_SPELL_EFFECTS);
    static uint256 CalcArmorReducedDamage(Unit const* attacker, Unit const* victim, uint256 const& damage, SpellInfo const* spellInfo, uint8 attackerLevel = 0, WeaponAttackType attackType = MAX_ATTACK);
    ...
    static uint256 SpellCriticalDamageBonus(Unit const* caster, SpellInfo const* spellProto, uint256 const& damage, Unit const* victim);
    ...
    */
    ...
    static void ProcDamageAndSpell(Unit* actor, Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint256 const& amount, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpellInfo = nullptr, SpellInfo const* procAura = nullptr, int8 procAuraEffectIndex = -1, Spell const* procSpell = nullptr, DamageInfo* damageInfo = nullptr, HealInfo* healInfo = nullptr, uint32 procPhase = 2 /*PROC_SPELL_PHASE_HIT*/);
    void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpellInfo, uint256 const& damage, SpellInfo const* procAura = nullptr, int8 procAuraEffectIndex = -1, Spell const* procSpell = nullptr, DamageInfo* damageInfo = nullptr, HealInfo* healInfo = nullptr, uint32 procPhase = 2 /*PROC_SPELL_PHASE_HIT*/);
    ...
    uint256 SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint256 const& healamount, DamageEffectType damagetype, uint8 effIndex, float TotalMod = 0.0f, uint32 stack = 1);
    uint256 SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint256 const& healamount, DamageEffectType damagetype, uint32 stack = 1);
    static uint256 SpellCriticalHealingBonus(Unit const* caster, SpellInfo const* spellProto, uint256 const& damage, Unit const* victim);

    static void CalcAbsorbResist(DamageInfo& dmgInfo, bool Splited = false);
    static void CalcHealAbsorb(HealInfo& healInfo);

    // Energize spells
    void SendEnergizeSpellLog(Unit* victim, uint32 SpellID, uint256 const& Damage, Powers powertype, uint256 const& effectiveGain = 0);
    void EnergizeBySpell(Unit* victim, uint32 SpellID, uint256 const& Damage, Powers powertype);
...
namespace Acore
{
    ...
    public:
        ...
        bool operator()(WorldObject const* objA, WorldObject const* objB) const
        {
            ...
            uint256 maxPowerA = a ? a->GetMaxPowerForCombat256(_power) : 0;
            uint256 maxPowerB = b ? b->GetMaxPowerForCombat256(_power) : 0;
            long double rA = maxPowerA != 0 ? Acore::Number::ToLongDouble(a->GetPowerForCombat256(_power)) / Acore::Number::ToLongDouble(maxPowerA) : 0.0L;
            long double rB = maxPowerB != 0 ? Acore::Number::ToLongDouble(b->GetPowerForCombat256(_power)) / Acore::Number::ToLongDouble(maxPowerB) : 0.0L;
            return _ascending ? rA < rB : rA > rB;
        }

        bool operator()(Unit const* a, Unit const* b) const
        {
            uint256 maxPowerA = a->GetMaxPowerForCombat256(_power);
            uint256 maxPowerB = b->GetMaxPowerForCombat256(_power);
            long double rA = maxPowerA != 0 ? Acore::Number::ToLongDouble(a->GetPowerForCombat256(_power)) / Acore::Number::ToLongDouble(maxPowerA) : 0.0L;
            long double rB = maxPowerB != 0 ? Acore::Number::ToLongDouble(b->GetPowerForCombat256(_power)) / Acore::Number::ToLongDouble(maxPowerB) : 0.0L;
            return _ascending ? rA < rB : rA > rB;
        }
    ...
    public:
        ...
        bool operator()(WorldObject const* objA, WorldObject const* objB) const
        {
            Unit const* a = objA->ToUnit();
            Unit const* b = objB->ToUnit();
            uint256 maxHealthA = a ? a->GetMaxHealthForCombat256() : 0;
            uint256 maxHealthB = b ? b->GetMaxHealthForCombat256() : 0;
            float rA = (a && maxHealthA != 0) ? float(Acore::Number::ToLongDouble(a->GetHealthForCombat256()) / Acore::Number::ToLongDouble(maxHealthA)) : 0.0f;
            float rB = (b && maxHealthB != 0) ? float(Acore::Number::ToLongDouble(b->GetHealthForCombat256()) / Acore::Number::ToLongDouble(maxHealthB)) : 0.0f;
            return _ascending ? rA < rB : rA > rB;
        }

        bool operator() (Unit const* a, Unit const* b) const
        {
            uint256 maxHealthA = a->GetMaxHealthForCombat256();
            uint256 maxHealthB = b->GetMaxHealthForCombat256();
            float rA = maxHealthA != 0 ? float(Acore::Number::ToLongDouble(a->GetHealthForCombat256()) / Acore::Number::ToLongDouble(maxHealthA)) : 0.0f;
            float rB = maxHealthB != 0 ? float(Acore::Number::ToLongDouble(b->GetHealthForCombat256()) / Acore::Number::ToLongDouble(maxHealthB)) : 0.0f;
            return _ascending ? rA < rB : rA > rB;
        }
