...
struct boss_faction_championsAI : public ScriptedAI
{
    ...
    float GetThreatMod(float dist, float  /*armor*/, uint32 health, uint32 /*maxhealth*/, Unit* target)
    {
        ...
        float mod_armor = (mAIType == AI_MELEE || mAIType == AI_PET) ? Acore::Number::ToLongDouble(Unit::CalcArmorReducedDamage(me, target, 10000, nullptr)) / 10000.0L : 1.0f;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (me->getPowerType() == POWER_MANA )
        {
            if (powerTimer <= diff)
            {
                me->ModifyPower128(POWER_MANA, Acore::Number::ToInt128Saturated(me->GetMaxPowerForCombat128(POWER_MANA) / 3));
                powerTimer = 4000;
            }
            else
                powerTimer -= diff;
        ...
        else if (me->getPowerType() == POWER_ENERGY )
        {
            if (powerTimer <= diff)
            {
                me->ModifyPower128(POWER_ENERGY, Acore::Number::ToInt128Saturated(me->GetMaxPowerForCombat128(POWER_ENERGY) / 3));
                powerTimer = 1000;
            }
            else
                powerTimer -= diff;
