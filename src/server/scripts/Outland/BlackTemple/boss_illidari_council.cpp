...
struct boss_illidari_council_memberAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit* /*attacker*/, uint256& damage, DamageEffectType /*damagetype*/, SpellSchoolMask /*damageSchoolMask*/) override
    {
        ...
        int32 damageTaken = damage > static_cast<uint256>(std::numeric_limits<int32>::max()) ? std::numeric_limits<int32>::max() : static_cast<int32>(damage);
...
class spell_illidari_council_circle_of_healing : public SpellScript
{
    ...
    void HandleSharedRule(SpellEffIndex /*effIndex*/)
    {
        ...
        uint256 heal = GetHitHeal256();
        int32 healBp = heal > static_cast<uint256>(std::numeric_limits<int32>::max()) ? std::numeric_limits<int32>::max() : static_cast<int32>(heal);
