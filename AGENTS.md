...
- 固定 WoW 3.3.5 客户端根目录为 `E:\World of Warcraft335`，客户端可执行文件为 `E:\World of Warcraft335\wow.exe`；涉及客户端注入、DLL 验证、客户端 `Errors` 崩溃日志、插件素材或补丁打包时默认使用该路径，不要每次重新查找或向用户确认。
